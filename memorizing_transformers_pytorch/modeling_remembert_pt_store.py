...
"""
Copied from Transformer's PyTorch BERT model.
Extend Bert with static, parameter-free KNN-Memory.
TODOS:
    - Make memory work.
    - Adapt hf_adapter to work with BertEncoder.
"""
...
from torchpq.index import IVFPQIndex, FlatIndex
...
class RememBertKNNSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        self.normalize_memories = config.normalize_memories
        ...
        self.stores = [
            FlatIndex(
                d_vector=self.attention_head_size,
                n_subvectors=64,
                n_cells=1024,
                initial_size=2048,
                distance="cosine",
...
class RememBertEncoder(nn.Module):
    def __init__(self, config):
        ...
        for layer_idx in range(config.num_hidden_layers):
            if layer_idx in config.memorizing_layers:
